...
public class FullScreenPlugin extends CordovaPlugin {
	...
	public static final String ACTION_GET_FULLSCREEN_STATUS = "getFullscreenStatus";
	...
	public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
		...
		if (ACTION_IS_SUPPORTED.equals(action))
			...
			return resetScreen();
		else if (ACTION_GET_FULLSCREEN_STATUS.equals(action))
	...
	private void setStatusBarBackgroundColor(final String colorPref) {
		if (Build.VERSION.SDK_INT >= 21) {
			if (colorPref != null && !colorPref.isEmpty()) {
				...
				// Method and constants not available on all SDKs but we want to be able to
				// compile this code with any SDK
	...
	/**
	 * Get SystemUi Visibility
	 */
	protected boolean getFullscreenStatus() {
		activity.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				try {
					int visibility = decorView.getSystemUiVisibility();
					// Log.i("CordovaPlugin", "Visibility: "+visibility);

					PluginResult res = new PluginResult(PluginResult.Status.OK, visibility);
					context.sendPluginResult(res);
				} catch (Exception e) {
					context.error(e.getMessage());
				}
